...
static void rebuild_flat_triangle_array(t_scene *scene)
{
    ...
    for (uint32_t m = 0; m < scene->mesh_count; m++)
    {
        t_mesh *mesh = &scene->meshes[m];
        memcpy(scene->triangles + offset,
               mesh->triangles,
               sizeof(t_triangle) * mesh->triangle_count);
...
uint32_t scene_add_mesh(t_scene *scene, t_mesh mesh)
{
    if (scene->mesh_count == scene->mesh_capacity)
    {
        ...
        scene->meshes = realloc(scene->meshes,
                                sizeof(t_mesh) * scene->mesh_capacity);
        scene->descriptors = realloc(scene->descriptors,
                                     sizeof(t_mesh_descriptor) * scene->mesh_capacity);
    }

    uint32_t index = scene->mesh_count++;
    scene->meshes[index] = mesh;

    scene->descriptors[index] = (t_mesh_descriptor){
...
void scene_upload_triangles(t_scene *scene)
{
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER,
                 sizeof(t_triangle) * scene->triangle_count,
                 scene->triangles,
                 GL_STATIC_DRAW);
...
void scene_upload_descriptors(t_scene *scene)
{
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER,
                 sizeof(t_mesh_descriptor) * scene->mesh_count,
                 scene->descriptors,
                 GL_DYNAMIC_DRAW);
