...
static void	render_frame(t_cycles cycles, GLint loc_resolution, GLint loc_mesh_count,
		GLint loc_accumulation_tex_fs, t_scene scene)
{
		...
		glBindImageTexture(0, cycles.tex, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F);
...
int	main(void)
{
	...
	loc_resolution = glGetUniformLocation(cycles.compute_program, "u_resolution");
	loc_mesh_count = glGetUniformLocation(cycles.compute_program, "u_mesh_count");
	loc_accumulation_tex_fs = glGetUniformLocation(cycles.fullscreen_program, "u_accumulation_tex");
	while (!glfwWindowShouldClose(cycles.win))
	{
		...
		render_frame(cycles, loc_resolution, loc_mesh_count, loc_accumulation_tex_fs, scene);
		glfwSwapBuffers(cycles.win);
		glfwPollEvents();
	}
	scene_destroy(&scene);
