...
int main()
{
	...
	t_scene scene = scene_create(8);
	t_mesh sphere1 = generate_uv_sphere(32, 32, 1.0f);
	sphere1.position = (t_vec4){0.0f, 0.0f, -3.0f, 0.0f};
	t_mesh sphere2 = generate_uv_sphere(100, 100, 0.5f);
	sphere2.position = (t_vec4){2.0f, 0.5f, -4.0f, 0.0f};
	t_mesh sphere3 = generate_uv_sphere(100, 100, 0.5f);
	sphere3.position = (t_vec4){-2.0f, 0.5f, -4.0f, 0.0f};
	uint32_t idx1 = scene_add_mesh(&scene, sphere1);
	uint32_t idx2 = scene_add_mesh(&scene, sphere2);
    ...
    GLint loc_mesh_count =
        glGetUniformLocation(compute_program, "u_mesh_count");

    while (!glfwWindowShouldClose(window))
    {
        time += 0.016f;

        scene_move_mesh(&scene, idx1,
                        (t_vec4){sinf(time) * 2.0f, 0.0f, -3.0f, 0.0f});
        ...
        glBindTexture(GL_TEXTURE_2D, tex);
        glUniform1i(
            glGetUniformLocation(fullscreen_program, "u_accumulation_tex"), 0);
